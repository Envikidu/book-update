...
int store_books()                         //把图书链表储存在文件中 （覆盖） 
{
	...
	//printf("%s",Book_Head.array->title);
...
int load_books()                  //从文件中取出图书链表 
{
	...
	for(i=0;i<Book_Head.length;i++)
	{
		...
		char read2[LEN];
		new_book->title=(char*)malloc(sizeof(char));
		fgets(read,LEN,fp);
		sscanf(read,"Title:\%[^\n]",read2);
		strcpy(new_book->title,read2);
		
		new_book->authors=(char*)malloc(sizeof(char));
		fgets(read,LEN,fp);
		sscanf(read,"Authors:\%[^\n]",read2);
...
int store_users()    //存储用户到文件 (覆盖)
{
	...
	while(z!=NULL)  //逐行写入txt 
	{
		...
		int i=0;  //借阅数组底标 
		//printf("借书量 %d\n",z->borrow_number);
		while(z->borrow_number!=0)  //列出目前在借的书的id，每行一个ID号 
		{	
			//printf("%d ",z->borrow_number);
			if(vice==z->borrow_number)  //在借的书全部遍历完毕 
			{
				break;
			}
...
int load_users()
{
	...
	for(i=0;i<User_Head.number;i++)
	{
		...
		fgets(read,LEN,fp);
		sscanf(read,"Realname:\%[^\n]",read2);
		strcpy(new_user->real_name,read2);
		...
		if(i==0)  //在向头部连接第一个节点时 
		{
			...
			int u=new_user->borrow_number;  
			for(u;u<99-(new_user->borrow_number)-1;u++)  //初始化借书数组，把未借书 
			{
				new_user->borrowed[u]=-1;
			}
			continue; 
		}

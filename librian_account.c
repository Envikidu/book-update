	...
	while(1)
	{
		...
		{	//printf("kk");
			if(id_number==search_before_add->id)  //书库里有这本书 
			{
				...
				printf("%-10d",search_before_add->id);
				printf("%-30s",search_before_add->title);
				printf("%-30s",search_before_add->authors);
				printf("%-15d",search_before_add->year);
				printf("%-10d",search_before_add->copies);
		...
		{	
			...
			//gets(add_book_author);
			if(strspn(add_book_author, "0123456789")!=0)
			{
			...
			while(1)
			{
	        	...
	        	{
	    			printf("Only numbers are valid characters/n");
	    			return;
		    	}
		...
		//	}
			...
			book_end->authors=(char*)malloc(sizeof(char));
			book_end->title=(char*)malloc(sizeof(char));
